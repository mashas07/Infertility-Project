...
def plot_patient_distribution(data_column, patient_value, column_name):
    ...
    ax.hist(data_column, bins=20, alpha=0.5, color='mediumseagreen', edgecolor='black', density=True,
            label='Population')
    ...
    ax.axvline(patient_value, color='red', linestyle='--', linewidth=2,
               label=f'Patient ({column_name}: {patient_value})')

    percentile = stats.percentileofscore(data_column, patient_value)
    ax.text(patient_value, ax.get_ylim()[1] * 0.9, f' {percentile:.1f}th percentile', ha='center', fontsize=12,
